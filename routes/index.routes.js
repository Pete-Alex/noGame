  ...
  (async () => {
    try {
      ...
      res.render("add-new-building", {
        planetInfo: planetObj,
        buildingTypesInfo: buildingTypesObj,
      });
    } catch (e) {
      console.log("error", e);
    }
...
router.post("/buildings/:buildingId/harvest", (req, res, next) => {
  ...
  (async () => {
    try {
      ...
      req.session.currentUser.ressources.metal +=
        statsBuilding.production.metal;
      req.session.currentUser.ressources.energy +=
        statsBuilding.production.energy;
